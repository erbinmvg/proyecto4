...
import { COLECCION, saveData, readData } from './reservaProvider';
const mail = React.createRef();
const nombre = React.createRef();
const apellidos = React.createRef();
const fono = React.createRef();
const nPersonas = React.createRef();
const fechaHr = React.createRef();
const mensaje = React.createRef();

function EnvioReserva() {

  const [setCltes] = React.useState([]);

  const addCltes = async () => {
    await saveData(COLECCION, nombre.current.value,apellidos.current.value,fono.current.value, mail.current.value, nPersonas.current.value, fechaHr.current.value, mensaje.current.value)
      .then(() => {
        readData(COLECCION)
          .then((data) => {
            setCltes(data);
    ...
    <Row className="justify-content-md-center">
      <Col xs lg="6">
        <Form>
          ...
          <Form.Group className="mb-3" id="formNick">
            <Form.Control ref={nPersonas} type="text" placeholder="Cantidad de Personas"></Form.Control>
          </Form.Group>

          <Form.Group className="mb-3" id="formNick">
            <Form.Control ref={fechaHr} type="text" placeholder="Frcha y Hora de la Reserva"></Form.Control>
          </Form.Group>
